...
async function makePack(valid:boolean):Promise<string>{
  ...
  const evals=cats.map((category,i)=>({name:`case-${i}`,category,input:"test",expect:["observable behaviour"],forbid:[]}));
